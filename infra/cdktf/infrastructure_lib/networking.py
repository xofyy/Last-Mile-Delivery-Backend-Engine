...
class StandardVPC(Construct):
    def __init__(self, scope: Construct, id: str, config: NetworkConfig):
        ...
        self.subnet = ComputeSubnetwork(self, "subnet",
            ...
            secondary_ip_range=[
                ComputeSubnetworkSecondaryIpRange(range_name="pod-ranges", ip_cidr_range=config.pod_cidr),
                ComputeSubnetworkSecondaryIpRange(range_name="service-ranges", ip_cidr_range=config.service_cidr)
            ]
        )
