...
class StandardIdentity(Construct):
    def __init__(self, scope: Construct, id: str, project_id: str, sa_id: str, k8s_namespace: str, k8s_sa_name: str):
        super().__init__(scope, id)

        # 1. Create Google Service Account (GSA)
        gsa = ServiceAccount(self, "sa",
            account_id=sa_id,
            display_name=f"Workload Identity SA for {sa_id}"
        )

        # 2. Grant GSA Access to Secrets
        ProjectIamMember(self, "secret_accessor",
            project=project_id,
            role="roles/secretmanager.secretAccessor",
            member=f"serviceAccount:{gsa.email}"
        )
